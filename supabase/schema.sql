...
create or replace function public.consume_capture_entitlement(p_user uuid, p_trial_key text)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  insert into public.free_trial_claims (trial_key, user_id)
  values (p_trial_key, p_user)
  ...
  values (p_user, 'free_capture_claimed', jsonb_build_object('trial_key', p_trial_key));

  return jsonb_build_object('allowed', true, 'reason', 'free', 'plan', 'free');
end;
$$;

revoke all on function public.consume_capture_entitlement(uuid,text) from public;
grant execute on function public.consume_capture_entitlement(uuid,text) to service_role;
